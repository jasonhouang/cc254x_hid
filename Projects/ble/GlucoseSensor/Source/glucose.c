/******************************************************************************
 ...
 @brief This file contains the Glucose Sensor sample application for use with
        the CC2540 Bluetooth Low Energy Protocol Stack.
...
static void glucose_ProcessGattMsg( gattMsgEvent_t *pMsg );
...
static void glucoseCtlPntIndicate(uint8 opcode, uint8 oper, 
                                  uint8 value0, uint8 value1);
...
static void glucose_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
  ...
  case GATT_MSG_EVENT:
      glucose_ProcessGattMsg( (gattMsgEvent_t *)pMsg );
      break;
...
static void glucoseCtlPntResponse(uint8 rspCode, uint8 opcode)
{
  glucoseCtlPntIndicate(CTL_PNT_OP_REQ_RSP, CTL_PNT_OPER_NULL, opcode, rspCode);
}
...
static void glucoseCtlPntNumRecordsResponse(uint16 numRecords)
{
  glucoseCtlPntIndicate(CTL_PNT_OP_NUM_RSP, CTL_PNT_OPER_NULL,
                        LO_UINT16(numRecords), HI_UINT16(numRecords));
}

/*********************************************************************
 * @fn      glucoseCtlPntIndicate
 *
