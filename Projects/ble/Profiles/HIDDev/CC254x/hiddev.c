...
static uint8 hidDevSendNoti( uint16 handle, uint8 len, uint8 *pData );
    ...
    case HIDDEV_ERASE_ALLBONDS:
      if ( len == 0 )
      {
        ...
        if ( (pRpt = hidDevRptById( lastReport.id, lastReport.type )) != NULL )
        {
          ...
          if ( osal_isbufset( lastReport.data, 0x00, lastReport.len ) == FALSE )
          {
            // Send a release report before disconnecting, otherwise
            // the last pressed key would get 'stuck' on the HID Host.
            osal_memset( lastReport.data, 0x00, lastReport.len );

            // Send report notification
            VOID hidDevSendNoti( pRpt->handle, lastReport.len, lastReport.data );
          }
          
          // Clear out last report
          VOID osal_memset( &lastReport, 0, sizeof(hidDevReport_t) );
        }
...
static void hidDevSendReport( uint8 id, uint8 type, uint8 len, uint8 *pData )
{
  ...
  {
    uint8 value = GATTServApp_ReadCharCfg(gapConnHandle, 
    ...
    if ( value & GATT_CLIENT_CFG_NOTIFY )
    {
      ...
      // Send report notification
      if ( hidDevSendNoti( pRpt->handle, len, pData ) == SUCCESS )
      {
        // Save the report just sent out
        lastReport.id = id;
        lastReport.type = type;
        lastReport.len = len;
        VOID osal_memcpy( lastReport.data, pData, len );
      }
