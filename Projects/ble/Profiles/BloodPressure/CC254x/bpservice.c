 ...
 * LOCAL FUNCTIONS
 */
static bStatus_t bloodPressure_ReadAttrCB( uint16 connHandle, gattAttribute_t *pAttr, 
...
bStatus_t BloodPressure_AddService( uint32 services )
{
  ...
  bloodPressureMeasConfig = (gattCharCfg_t *)osal_mem_alloc( sizeof(gattCharCfg_t) *
                                                             linkDBNumConns );
  ...
  // Allocate Client Characteristic Configuration table
  bloodPressureIMeasConfig = (gattCharCfg_t *)osal_mem_alloc( sizeof(gattCharCfg_t) *
                                                              linkDBNumConns );
  if ( bloodPressureIMeasConfig == NULL )
  {
    // Free already allocated data
    osal_mem_free( bloodPressureMeasConfig );
