...
static bStatus_t running_ReadAttrCB( uint16 connHandle, gattAttribute_t *pAttr,
...
uint16 RunningService_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    if ( GATT_Indication( connectionHandle, &rscCmdInd, FALSE,
                          runningService_TaskID ) != SUCCESS )
    {
      GATT_bm_free( (gattMsg_t *)&rscCmdInd, ATT_HANDLE_VALUE_IND );
    }

    // Clear out this indication.
    osal_memset( &rscCmdInd, 0, sizeof(attHandleValueInd_t) );
...
bStatus_t Running_AddService( uint32 services )
{
  ...
  runningMeasClientCharCfg = (gattCharCfg_t *)osal_mem_alloc( sizeof(gattCharCfg_t) *
                                                              linkDBNumConns );
  ...
  // Allocate Client Characteristic Configuration table
  runningCommandClientCharCfg = (gattCharCfg_t *)osal_mem_alloc( sizeof(gattCharCfg_t) *
                                                                 linkDBNumConns );
  if ( runningCommandClientCharCfg == NULL )
  {
    // Free already allocated data
    osal_mem_free( runningMeasClientCharCfg );
  ...
  switch ( pValue[0] )
  {
    ...
    case RSC_REQ_SUPP_SENS_LOC:
      ...
      if ( ( len == 1 ) && ( runningFeatures & RSC_MULTI_SENS_SUPP ) )
      {
        ...
        osal_memcpy( &(rscCmdInd.pValue[3]), supportedSensorLocations, supportedSensors );
      }
      else // characteristic not supported.
      {
        // Send an indication with the list.
