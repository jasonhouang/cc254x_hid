 ...
 * LOCAL FUNCTIONS
 */
static uint8 gapCentralRole_ProcessOSALMsg( osal_event_hdr_t *pMsg );
static void gapCentralRole_ProcessGattEvent(gattMsgEvent_t *pMsg);
static uint8 gapCentralRole_ProcessGAPMsg( gapEventHdr_t *pMsg );
...
uint16 GAPCentralRole_ProcessEvent( uint8 taskId, uint16 events )
{
  if ( events & SYS_EVENT_MSG )
  {
    ...
    {
      if ( gapCentralRole_ProcessOSALMsg( (osal_event_hdr_t *) pMsg ) )
      {
...
static uint8 gapCentralRole_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
  ...
  switch ( pMsg->event )
  {
    ...
    case GATT_MSG_EVENT:
      // Process GATT event
      gapCentralRole_ProcessGattEvent((gattMsgEvent_t *)pMsg);
      break;
...
/*********************************************************************
 * @fn      gapCentralRole_ProcessGattEvent
 *
