/******************************************************************************
 ...
 @brief This file contains the OAD Manager sample application for use with the
        CC2540 Bluetooth Low Energy Protocol Stack.
...
// Reject Connection Parameter Update request received from Peripheral device
#define DEFAULT_OAD_REJECT_CONN_PARAMS        TRUE
...
static uint8 oadManagerEventCB( gapCentralRoleEvent_t *pEvent );
#if (defined HAL_KEY) && (HAL_KEY == TRUE)
...
static void oadManagerSvcDiscoveryMsg(gattMsgEvent_t *pMsg)
{
  if ( pMsg->hdr.status == SUCCESS ) // Characteristic found, the store handle.
  {
    ...
    if ( pRsp->numInfo > 0 )
    {
      oadSvcStartHdl = ATT_ATTR_HANDLE(pRsp->pHandlesInfo, 0);
      oadSvcEndHdl = ATT_GRP_END_HANDLE(pRsp->pHandlesInfo, 0);
    ...
    if ( pMsg->hdr.status == SUCCESS ) // CCCD found, the store handle.
    {
      ...
      if ( ( pRsp->numInfo > 0 ) && ( pRsp->format == ATT_HANDLE_BT_UUID_TYPE ) )
      {
        ...
        for ( uint8 i = 0; i < pRsp->numInfo; i++ )
        {
          ...
          {
            // CCCD found
            oadManagerCCCHandles[oadManagerCCCIdx] = 
...
static void oadManagerCharDiscoveryMsg(gattMsgEvent_t *pMsg)
{
  if (oadManagerDiscIdx < OAD_CHAR_CNT)
  {
    if ( pMsg->hdr.status == SUCCESS ) // Characteristic found, the store handle.
    {
      ...
      if (pRsp->numPairs > 0)
      {
        oadManagerHandles[oadManagerDiscIdx] = BUILD_UINT16(pRsp->pDataList[3],
                                                            pRsp->pDataList[4]);
...
static void oadManagerEnableNoti( uint16 handle )
{
  ...
  req.pValue = GATT_bm_alloc(oadManagerConnHandle, ATT_WRITE_REQ, 2, NULL);
  if ( req.pValue != NULL )
  {
    req.handle = handle;
    req.len = 2;
    ...
    if (GATT_WriteNoRsp(oadManagerConnHandle, &req) != SUCCESS)
    {
...
static void oadManagerHandleNoti(attHandleValueNoti_t *pNoti)
{
  ...
  {
#if (defined HAL_LCD && (HAL_LCD == TRUE))
    uint16 ver = BUILD_UINT16(pNoti->pValue[0], pNoti->pValue[1]);
    uint8 userId[12] = "UserId ";

    osal_memcpy(&(userId[7]), &(pNoti->pValue[4]), 4);
    userId[11] = '\0';

    if ( OAD_IMG_ID( ver ) == 0 )
    {
        HalLcdWriteStringValueValue("ImgA Id", OAD_VER_NUM( ver ), 16,
                                               BUILD_UINT16(pNoti->pValue[2], pNoti->pValue[3]), 16,
                                               HAL_LCD_LINE_2);
    }
    else
    {
  ...
  else if (pNoti->handle == oadManagerHandles[OAD_CHAR_IMG_BLOCK])
  {
    ...
    req.pValue = GATT_bm_alloc(oadManagerTaskId, ATT_WRITE_REQ, 
                               2 + OAD_BLOCK_SIZE, NULL);
    if (req.pValue != NULL)
    {
      oadBlkNum = BUILD_UINT16(pNoti->pValue[0], pNoti->pValue[1]);
      ...
      req.pValue[0] = LO_UINT16(oadBlkNum);
      req.pValue[1] = HI_UINT16(oadBlkNum);

      uint8 page = oadBlkNum / OAD_BLOCKS_PER_PAGE;
      uint16 oset = (oadBlkNum - (OAD_BLOCKS_PER_PAGE * page)) * OAD_BLOCK_SIZE;
      HalFlashRead(page+OAD_IMG_B_PAGE, oset, req.pValue+2, OAD_BLOCK_SIZE);
...
static void oadManagerSendImgNotify(void)
{
  ...
  req.pValue = GATT_bm_alloc(oadManagerConnHandle, ATT_WRITE_REQ, 
                             OAD_IMG_HDR_SIZE, NULL);
  if ( req.pValue != NULL )
  {
    img_hdr_t ImgHdr;
    ...
    req.pValue[0] = LO_UINT16(ImgHdr.ver);
    req.pValue[1] = HI_UINT16(ImgHdr.ver);

    req.pValue[2] = LO_UINT16(ImgHdr.len);
    req.pValue[3] = HI_UINT16(ImgHdr.len);

    (void)osal_memcpy(req.pValue+4, ImgHdr.uid, sizeof(ImgHdr.uid));
