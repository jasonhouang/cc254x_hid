/******************************************************************************
 ...
 @file  hci_ext_appGeneric.c

 @brief HCI Extensions Application
...
#include "osal_snv.h"
  ...
  #if defined(DEVICE_TYPE) && (DEVICE_TYPE & LL_DEV_TYPE_SLAVE)
...
#define HCI_SEND_KEY_EVENT               0x0002
...
#if !defined ( GATT_DB_OFF_CHIP )
  #if defined(DEVICE_TYPE) && (DEVICE_TYPE & LL_DEV_TYPE_SLAVE)
    static uint8 deviceName[GAP_DEVICE_NAME_LEN] = "TI BLE Slave";
  #else
    static uint8 deviceName[GAP_DEVICE_NAME_LEN] = "TI BLE Master";
  #endif
  static uint16 appearance = 17;
#endif

static uint8 lastKeyValue = 0;
...
static uint8 processExMsgGATT( uint8 cmdID, hciExtCmd_t *pCmd );
...
static uint8 sendGenericKey( uint8 key );
...
void HCI_EXT_App_Init( uint8 task_id )
{
  ...
  VOID GGS_SetParameter( GGS_APPEARANCE_ATT, 2, (void*)&appearance );
#else
  // Register with GATT Server for GATT messages
  GATT_RegisterForMsg( hciExtApp_TaskID );
#endif // GATT_DB_OFF_CHIP
...
uint16 HCI_EXT_App_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & HCI_SEND_KEY_EVENT )
  {
    VOID sendGenericKey( lastKeyValue );

    return ( events ^ HCI_SEND_KEY_EVENT );
  }
...
static uint8 processExtMsg( hciPacket_t *pMsg )
{
  ...
  switch( msg.opCode >> 7 )
  {
    ...
    case HCI_EXT_GATT_SUBGRP:
      stat = processExMsgGATT( (msg.opCode & 0x007F), &msg );
      break;
  ...
  *pRspDataLen = 0;
  ...
  switch( cmdID )
  {
    case HCI_EXT_UTIL_RESET:
      if ( pBuf[0] == 0 )
      {
        VOID osal_start_timerEx( hciExtApp_TaskID, HCI_EXT_RESET_EVENT, RESET_TIMEOUT );
      }
      else
      {
        stat = INVALIDPARAMETER;  // For now, until soft reset is working
...
static uint8 processExMsgATT( uint8 cmdID, hciExtCmd_t *pCmd )
{
  ...
  if ( stat == ATT_ERR_INVALID_PDU )
  {
    // Returned from Parse routines
    stat = bleInvalidPDU;
  }
  else if ( stat == ATT_ERR_INSUFFICIENT_AUTHEN )
  {
    // Returned from Send routines
    stat = bleInsufficientAuthen;
  }
  else if ( stat == ATT_ERR_INSUFFICIENT_KEY_SIZE )
  {
    // Returned from Send routines
    stat = bleInsufficientKeySize;
  }
  else if ( stat == ATT_ERR_INSUFFICIENT_ENCRYPT )
  {
    // Returned from Send routines
...
static uint8 processExMsgGATT( uint8 cmdID, hciExtCmd_t *pCmd )
{
  ...
  switch( cmdID )
  {
    ...
    case ATT_PREPARE_WRITE_REQ: // GATT Write Long
    ...
    case HCI_EXT_GATT_ADD_SERVICE:
      if ( service.attrs == NULL )
      {
             ...
             ( numAttrs > 0 ) )
        {
          ...
          if ( service.attrs != NULL )
          {
            gattAttribute_t *pAttr = &service.attrs[0];

            // Zero out all attribute fields
            osal_memset( service.attrs, 0, numAttrs * sizeof( gattAttribute_t ) );

            // Set up service record
            pAttr->type.uuid = GATT_FindUUIDRec( pBuf, ATT_BT_UUID_SIZE );
            pAttr->type.len = ATT_BT_UUID_SIZE;
            pAttr->permissions = GATT_PERMIT_READ;
    ...
    case HCI_EXT_GATT_DEL_SERVICE:
      {
        ...
        else
        {
          ...
          if ( ( stat == SUCCESS ) && ( serv.attrs != NULL ) )
          {
            osal_mem_free( serv.attrs );
          }
        }
      }
      break;
    ...
    case HCI_EXT_GATT_ADD_ATTRIBUTE:
      if ( service.attrs != NULL )
      {
        gattAttribute_t *pAttr = &service.attrs[service.numAttrs];

        // Set up attribute record
        pAttr->type.uuid = GATT_FindUUIDRec( pBuf, pCmd->len-1 );
        if ( pAttr->type.uuid != NULL )
        {
          pAttr->type.len = pCmd->len-1;
          pAttr->permissions = pBuf[pCmd->len-1];

          // Are all attributes added to the service yet?
          if ( ++service.numAttrs == totalAttrs )
          {
            // Register the service with the GATT Server
            stat = GATT_RegisterService( &service );
            if ( stat != SUCCESS )
            {
              osal_mem_free( service.attrs );
            }
...
#if defined(DEVICE_TYPE) && ((DEVICE_TYPE & LL_DEV_TYPE_SLAVE) != LL_DEV_TYPE_SLAVE)
        if ( profileRole & (GAP_PROFILE_PERIPHERAL | GAP_PROFILE_BROADCASTER) )
        {
          stat = INVALIDPARAMETER;
          break;
        }
#endif
#if defined(DEVICE_TYPE) && ((DEVICE_TYPE & LL_DEV_TYPE_MASTER) != LL_DEV_TYPE_MASTER)
        if ( profileRole & (GAP_PROFILE_CENTRAL | GAP_PROFILE_OBSERVER) )
        {
          stat = INVALIDPARAMETER;
          break;
        }
    ...
    case HCI_EXT_GAP_BOND:
      {
        ...
        stat = GAP_Bond( connectionHandle, authenticated, &securityInfo );
      }
      break;
    ...
    case HCI_EXT_GAP_SET_PARAM:
      {
        ...
        {
          lastKeyValue = value;
          osal_start_timerEx( hciExtApp_TaskID, HCI_SEND_KEY_EVENT, 3000 );
        }
#endif // TESTMODES
        else
        {
...
#if defined ( GAP_BOND_MGR )
  #if defined(DEVICE_TYPE) && (DEVICE_TYPE & LL_DEV_TYPE_SLAVE)
        stat = GAPBondMgr_SetParameter( BUILD_UINT16( pBuf[0], pBuf[1] ), pBuf[2], &pBuf[3] );
  #else
        stat = INVALIDPARAMETER;
  #endif
#else
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
       ...
       if ( pPkt->hdr.status == SUCCESS )
       {
          ...
          GAPBondMgr_LinkEst( pPkt->devAddrType, pPkt->devAddr, pPkt->connectionHandle );
       }
  #endif
#endif
...
static uint8 sendGenericKey( uint8 key )
{
  ...
  // Allocate a buffer
  sendData.CID = L2CAP_CID_GENERIC;
  sendData.len = 2;
  sendData.pPayload = (uint8 *)L2CAP_bm_alloc( sendData.len );

  if ( sendData.pPayload )
  {
    sendData.pPayload[0] = 0x03;
    sendData.pPayload[1] = key;

    stat = L2CAP_SendData( 0x00, &sendData );
