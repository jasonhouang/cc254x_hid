/******************************************************************************
 ...
 @file  timeapp_discovery.c

 @brief Time App service and characteristic discovery routines.
...
static uint8 TimeAppDiscRefTime( uint8 state, gattMsgEvent_t *pMsg )
{
  ...
  switch ( state )
  {
    case DISC_REF_TIME_START:  
      {
        uint8 uuid[ATT_BT_UUID_SIZE] = { LO_UINT16(REF_TIME_UPDATE_SERV_UUID),
...
static uint8 TimeAppDiscNwa( uint8 state, gattMsgEvent_t *pMsg )
{
  ...
  switch ( state )
  {
    case DISC_NWA_START:  
      {
        uint8 uuid[ATT_BT_UUID_SIZE] = { LO_UINT16(NWA_SERV_UUID),
...
static uint8 TimeAppDiscPhoneAlert( uint8 state, gattMsgEvent_t *pMsg )
{
  ...
  switch ( state )
  {
    case DISC_PAS_START:  
      {
        uint8 uuid[ATT_BT_UUID_SIZE] = { LO_UINT16(PHONE_ALERT_STS_SERV_UUID),
