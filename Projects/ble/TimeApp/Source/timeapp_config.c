...
uint8 timeAppConfigNext( uint8 state )
{
  ...
  else
  {
    ...
    writeReq.pValue = GATT_bm_alloc( timeAppConnHandle, ATT_WRITE_REQ, 2, NULL );
    if (writeReq.pValue != NULL)
    {
      ...
      if ( GATT_WriteCharValue( timeAppConnHandle, &writeReq, timeAppTaskId )!= SUCCESS )
      {
       ...
       (pMsg->hdr.status == SUCCESS) )
  {
    ...
    switch ( timeAppConfigList[state] )
    {
      ...
      case HDL_BATT_LEVEL_START:
        ...
        LCD_WRITE_STRING_VALUE( "Battery%", pMsg->msg.readRsp.pValue[0], 10, HAL_LCD_LINE_2 );
        break;
      ...
      case HDL_PAS_ALERT_START:
        ...
        LCD_WRITE_STRING_VALUE( "Phone Alert:", pMsg->msg.readRsp.pValue[0], 16, HAL_LCD_LINE_1 );
        break;
