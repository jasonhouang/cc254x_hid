/******************************************************************************
 ...
 @brief This file contains the time app sample application for use with the
...
static void timeApp_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_LEFT )
  {
         ...
         ( timeAppHdlCache[HDL_ALERT_NTF_CTRL] != 0 ) )
    {
      ...
      req.pValue = GATT_bm_alloc( timeAppConnHandle, ATT_WRITE_REQ, 2, NULL );
      if (req.pValue != NULL)
      {
        req.len = 2;
        ...
        // Send write request
        if ( GATT_WriteCharValue( timeAppConnHandle, &req, timeAppTaskId ) != SUCCESS )
        {
  ...
  {   
    ...
    {
      timeAppWriteCtrl( timeAppHdlCache[HDL_REF_TIME_UPD_CTRL], timeAppRefUpdateVal );
  ...
  {   
    ...
    {      
      timeAppWriteCtrl( timeAppHdlCache[HDL_PAS_CTRL], timeAppRingerCmd );
