/******************************************************************************
 ...
 @file  _hal_uart_spi.c

 @brief Implementation of a specialized SPI Transport driver for emulating a
        UART API.
        
        
        The SPI frame format is <SOF> <LEN> <DATA
        Byte 1> < DATA Byte2 >...<DATA Byte N> <FCS>
        
        5 GPIO lines are required: MOSI, MISO, SPI Clock, SPI CSn/MRDY, and
        SRDY. MRDY/CS = SPI Master Ready to send and serves as a SPI Slave
        wakeup from sleep. SRDY = SPI Slave Ready to send and serves as a SPI
        Master wakeup from sleep.
        
        The SRDY output from SPI Slave to SPI Master will transition from high
        to low if the SPI slave receives an MRDY falling edge ISR from the SPI
        Master. But, if the SPI slave has nothing to send, the SRDY line will
        be pulled high again.
        ...
        Master is asserting MRDY. Slave has parsed the
        SOF byte of a SPI packet, but has not received the entire packet yet.
        Note how important this case is to support a master that has to
        interrupt a SPI packet transmission in order to service a higher
        priority SPI slave (i.e. the master has executed the protocol to ensure
        that the SPI slave is awake, but cannot hold MRDY low for the entire,
        contiguous packet for having to service some other SPI slave
        mid-packet).
        
        The incoming bytes may not be just garbage bytes clocked out by the
        synchronous transfer, but instead be meaningful bytes of a Tx-packet
        from the other side.
